angular.module('starter').controller('editConformityCtrl', function($rootScope,$scope, $state, $ionicPopup, $ionicModal,
  workOrderSet, Model, workOrderPDFService, ionicDatePicker) {
  console.log($scope.actasDeConformidad);
  $scope.id = $state.params.id;

  $scope.getDoc = function(id, $rootScope){
      return $rootScope.actasDeConformidad.filter(
        function(doc){
          if(doc.id === id){
            console.log(doc);
          }
  ...
  $scope.save = function(data){
    for(var i = 0; i < $rootScope.actasDeConformidad.length;i++){
      if($rootScope.actasDeConformidad[i].id === data.id){
        $rootScope.actasDeConformidad[i] = data;
      }
    }
    $state.go('home');
    return;
